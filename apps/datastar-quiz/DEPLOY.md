...
**Where the `.bml` corpus comes from.** It is not in this repo. The apps were split out of
`bridge-bidding-system`, which still owns the system notes, and `deploy` reads the corpus from
`BRIDGE_SYSTEM_HOME` (default `~/docs/bridge/bridge-bidding-system`). That checkout must be present
to deploy. On the box the layout below is unchanged: the `.bml` files land at the deploy root, which
is exactly where `quiz.bml_docs_dir()`'s two-levels-up-from-`apps/quiz` fallback looks.
