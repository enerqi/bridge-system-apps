...
route_filter_preview :: proc(
	...
	return event_route(event, request, response, route_context, raw_state, build_filter_preview)
}

// Check the TYPED text without adopting it, and patch the status line.
@(private = "file")
build_filter_preview :: proc(resolved: Request_Context, request: ^http.Request, state: ^Stream_State) {
	s := resolved.session
	typed := resolved.uploaded.filterText.? or_else s.filter_text
	push_status(state, s, typed, "#filter-status")
}

route_topics_preview :: proc(
	event: http.Route_Event,
	request: ^http.Request,
	response: ^http.Response,
	route_context: http.Route_Context,
	...
	return event_route(event, request, response, route_context, raw_state, build_topics_preview)
}

@(private = "file")
build_topics_preview :: proc(resolved: Request_Context, request: ^http.Request, state: ^Stream_State) {
	s := resolved.session
	text := ticked_topics_text(resolved, request)
	push_status(state, s, text, "#topics-status")
}
...
route_topics_reset :: proc(
	...
	return event_route(event, request, response, route_context, raw_state, build_topics_reset)
}

@(private = "file")
build_topics_reset :: proc(resolved: Request_Context, request: ^http.Request, state: ^Stream_State) {
	s := resolved.session
	check := corpus_check(s)
	push_signals(state, topics_only_signals(s, check.parsed.topic_names))
	clear_topics_status(state)
}
...
route_filter_apply :: proc(
	...
	return event_route(event, request, response, route_context, raw_state, build_filter_apply)
}

@(private = "file")
build_filter_apply :: proc(resolved: Request_Context, request: ^http.Request, state: ^Stream_State) {
	s := resolved.session
	typed := resolved.uploaded.filterText.? or_else s.filter_text
	apply_filter(state, s, typed, "#filter-status")
}

route_topics_apply :: proc(
	event: http.Route_Event,
	request: ^http.Request,
	response: ^http.Response,
	route_context: http.Route_Context,
	...
	return event_route(event, request, response, route_context, raw_state, build_topics_apply)
}

@(private = "file")
build_topics_apply :: proc(resolved: Request_Context, request: ^http.Request, state: ^Stream_State) {
	s := resolved.session
	apply_filter(state, s, ticked_topics_text(resolved, request), "#filter-status")
}
...
@(private = "file")
apply_filter :: proc(state: ^Stream_State, s: ^session.Session, text: string, status_selector: string) {
	check := corpus.check_filter(s.system, text, engine.MAX_DIFFICULTY)
	canonical := strings.clone(check.parsed.canonical_text, session.store_allocator())
	...
	clear_topics_status(state)
	push_signals(state, render.bound_signals(s, choices_for(s.system), check.parsed.topic_names, response_arena()))
...
@(private = "file")
push_status :: proc(state: ^Stream_State, s: ^session.Session, text: string, selector: string) {
	check := corpus.check_filter(s.system, text, engine.MAX_DIFFICULTY)
	push_elements(
		state,
		render.filter_status(
...
@(private = "file")
ticked_topics_text :: proc(resolved: Request_Context, request: ^http.Request) -> string {
	payload := string(http.body_buffered(request))
	if payload == "" {
		if decoded, result := http.query_value_decoded(request, "datastar"); result == .Found {
	...
	out := strings.builder_make(0, 256, response_arena())
	for choice in choices_for(resolved.system) {
		if !signal_is_true(payload, choice.key) {
			continue
		}
...
@(private = "file")
signal_is_true :: proc(payload, key: string) -> bool {
	needle := strings.concatenate({`"`, key, `"`}, response_arena())
...
@(private = "file")
topics_only_signals :: proc(s: ^session.Session, active: []string) -> string {
	out := strings.builder_make(0, 512, response_arena())
