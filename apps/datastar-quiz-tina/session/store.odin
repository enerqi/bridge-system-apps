...
//
// No lock, and none needed: ONE isolate owns this map and Tina runs one isolate at a time on its
// shard. What used to be a call from a handler is now a message to that isolate -- `isolate.odin`.
package session
...
Store :: struct {
	...
	// Callers parked on a session somebody else is inside. See `store_protocol.odin`.
...
make_store :: proc(allocator := context.allocator) -> Store {
	return Store {
		sessions = make(map[Key]^Session, 64, allocator),
		waiters = make([dynamic]Waiter, 0, 64, allocator),
		last_sweep = time.now(),
		allocator = allocator,
	}
}
...
sweep_if_due :: proc(store: ^Store) {
	...
	for key, session in store.sessions {
		// A LEASED session is never swept, however old it looks: another shard is inside it right
		// now, holding the pointer this map would otherwise drop. See `isolate.odin`.
